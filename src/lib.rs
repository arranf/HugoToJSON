...
pub fn convert_to_json_and_write(settings: Settings) -> Result<(), HugotoJsonError> {
    ...
    let traverse_results = create_page_index(&settings)?;
    let index = serde_json::to_string(&traverse_results.page_index)?;

    let writing_location;
    
    match settings.output {
        Some(ref path) => writing_location = path.to_string_lossy().into_owned(),
        None => writing_location = String::from("stdout")
    }
    ...
    if traverse_results.error_count > 0 {
        ...
        debug!("Succesfully wrote index to {0}", writing_location);
        Ok(())
    }
}
